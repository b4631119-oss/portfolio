...
export default async function ProLabAcademyCaseStudy({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const screenshots = cover ? [cover, { src: "/projects/prolab-academy/1.webp", alt: content.screenshotAlts[0] }, { src: "/projects/prolab-academy/2.webp", alt: content.screenshotAlts[1] }] : [];
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 md:px-6 py-16 md:py-24 font-sans">
      ...
      {cover && <div className="mt-8 mb-12 md:mb-16 relative aspect-[16/10] w-full overflow-hidden rounded-[var(--radius)] border border-line bg-bg-elevated"><Image src={cover.src} alt={locale === "en" ? "PROlab Academy platform home page" : cover.alt} fill priority sizes="(min-width: 896px) 896px, 100vw" className="object-cover" /></div>}
      <header className="space-y-6 mb-16 md:mb-24"><div className="flex flex-wrap gap-2"><span className="font-mono text-xs text-muted uppercase tracking-wide">{content.role}</span><Badge variant="secondary" className="font-mono text-xs">{content.badge}</Badge></div><h1 className="font-sans font-bold text-ink text-4xl md:text-6xl lg:text-7xl leading-[1.05] tracking-tight">PROlab Academy</h1><p className="text-lg md:text-xl text-muted max-w-2xl leading-relaxed">{content.intro}</p><div className="flex flex-wrap gap-4"><Button asChild size="lg" className="shadow-none font-bold"><a href="https://www.prolab-academy.site/" target="_blank" rel="noopener noreferrer"><ExternalLink size={18} className="mr-2" aria-hidden="true" />{content.live}</a></Button><Button asChild size="lg" variant="outline" className="font-bold"><a href="https://github.com/b4631119-oss/academy-exam" target="_blank" rel="noopener noreferrer"><Github size={18} className="mr-2" aria-hidden="true" />{content.code}</a></Button></div></header>
      ...
      <div className="mb-16 md:mb-24"><ProjectScreenshots images={screenshots.slice(1)} label={content.screenshots} dictionary={dictionary} /></div>
      ...
      <footer className="pt-8 border-t border-line flex flex-wrap gap-4"><Button asChild size="lg" className="shadow-none font-bold"><a href="https://www.prolab-academy.site/" target="_blank" rel="noopener noreferrer"><ExternalLink size={18} className="mr-2" aria-hidden="true" />{content.live}</a></Button><Button asChild size="lg" variant="outline" className="font-bold"><a href="https://github.com/b4631119-oss/academy-exam" target="_blank" rel="noopener noreferrer"><Github size={18} className="mr-2" aria-hidden="true" />{content.code}</a></Button><Button asChild size="lg" variant="outline" className="font-bold"><Link href={`${prefix}/#work`}><ArrowLeft size={18} className="mr-2" aria-hidden="true" />{content.back}</Link></Button></footer>
    </article>
  );
}
