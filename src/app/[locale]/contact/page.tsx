...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> { const { locale: raw } = await params; const locale: Locale = isLocale(raw) ? raw : "ru"; const d = getDictionary(locale); return { title: d.pages.contactTitle, description: `${d.pages.contactDescription} — ${contact.email}.`, alternates: alternatesFor("/contact", locale) }; }
